...
class WebmentionController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    protected function infoAction()
    {
        ...
        if (
            ...
            (file_get_contents('php://input') == md5(
                    $GLOBALS['TYPO3_CONF_VARS']['SYS']['encryptionKey'].'/'.(new \Jkphl\Indieweb\Utility\Url(
            ...
            $response->sendHeaders();
            echo(new \Jkphl\Indieweb\Utility\Url(
                \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_REQUEST_URL')
            ));
        }
        exit;
    ...
    protected function pingAction()
    {
        ...
        if (!$this->configurationManager->isFeatureEnabled(
                'webmentions'
            ) || empty($_POST['source']) || empty($_POST['target'])) {
            $response->setStatus(400, 'Bad Request');
            $response->sendHeaders();
            ...
            if (intval($this->settings['webmentions']['async']) > 0) {
                $updateWebmention ? $this->webmentionRepository->update($webmention) : $this->webmentionRepository->add(
                    $webmention
                );
                $persistenceManager->persistAll();
                ...
                if ($webmention->validate(intval($this->settings['webmentions']['excerpt']))) {
                    ...
                    $updateWebmention ? $this->webmentionRepository->update(
                        $webmention
                    ) : $this->webmentionRepository->add($webmention);
                    $persistenceManager->persistAll();
    ...
    protected function _isValidTarget($target)
    {
        ...
        $info = \Jkphl\Indieweb\Utility\Curl::httpRequest(
            $target, array(), \Jkphl\Indieweb\Utility\Curl::POST, $token, false,
            array(CURLOPT_USERAGENT => self::INDIEWEB_CLIENT), $status
        );
        $info = strlen($info) ? @json_decode($info) : null;
        return (($info instanceof \stdClass) && !empty($info->status) && $info->status && !empty($info->id)) ? $this->pageRepository->findByUid(
            intval($info->id)
        ) : null;
    }
}